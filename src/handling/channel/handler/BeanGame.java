    ...
    public static final void BeanGame1(LittleEndianAccessor slea, MapleClient c) {
        ...
        switch (type) {
            ...
            case 3:
                ...
                c.getSession().write(Res_Pachinko.BeansJDCS(get进洞次数(), 加速旋转, 蓝, 绿, 红));
                break;
            case 4:
                //记录进洞次数的黄色豆豆。最多只有7个。
                gain进洞次数(-1);
                ...
                c.getSession().write(Res_Pachinko.BeansJDXZ(get进洞次数(), 第一排, 第三排, 第二排, 启动打怪效果, 中奖率, 加速旋转, 打中女皇出现特效A, 打中女皇出现特效B));
                if (第二排 != 9) {
                    if (Randomizer.nextInt(Integer.parseInt(黄金狗几率[0])) == Integer.parseInt(黄金狗几率[1]) && 黄金狗设置局数 == 0) {
                        黄金狗设置局数 = 1;
                        c.getSession().write(Res_Pachinko.BeansHJG((byte) 1));
                    } else {
                        黄金狗设置局数 = 0;
                        c.getSession().write(Res_Pachinko.BeansHJG((byte) 0));
                    }
                }
                break;
            case 5://应该是普通的怪物中奖 //移动以后三排一样的 这里应该是处理 打完以后出现的 看看是不是判断一排的代码
                ...
                if ((第一排 == 0 && 第三排 == 0 && 第二排 == 0)
                    ...
                    switch (第二排) {
                        ...
                        case 0://大白怪
                            if (Randomizer.nextInt(Integer.parseInt(白怪奖励[0])) > Randomizer.nextInt(Integer.parseInt(白怪奖励[1]))) {
                                ...
                                c.getSession().write(Res_Pachinko.BeansUP(itemId));
                            }
                            break;
                        case 1://紫色怪
                        case 3://粉色怪
                        ...
                        case 9://女皇
                            if (Randomizer.nextInt(Integer.parseInt(女皇奖励[4])) > Randomizer.nextInt(Integer.parseInt(女皇奖励[5]))) {
                                ...
                                c.getSession().write(Res_Pachinko.BeansUP(海洋帽子));
                    ...
                    if (chr.getMapId() == 809030000) {
                        ...
                        c.getSession().write(Res_Pachinko.BeansGameMessage(0x01, 0x01, notea));
                    }
                    if (黄金狗设置局数 > 0 && 第二排 == 9) {
                        黄金狗设置局数 = 0;
                        c.getSession().write(Res_Pachinko.BeansHJG((byte) 0));
                    }
                    c.getSession().write(Res_Pachinko.BeansZJgeidd(true, 奖励豆豆));
                }
                break;
            case 7:
                if (黄金狗设置局数 > 0) {
                ...
                c.getSession().write(Res_Pachinko.BeansZJgeidd(true, 0));
                break;
            case 0x0B:
        ...
        if (type == 0x0B || type
            ...
            c.getSession().write(Res_Pachinko.showBeans(beansInfo));
        }
    }
