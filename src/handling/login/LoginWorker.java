...
public class LoginWorker {
    ...
    public static void registerClient(final MapleClient c) {
        if (LoginServer.isAdminOnly() && !c.isGm()) {
            c.SendPacket(LoginPacket.CheckPasswordResult(c, LoginResult.INVALID_ADMIN_IP));
            return;
        }
        ...
        if (System.currentTimeMillis() - lastUpdate > 600000) { // Update once every 10 minutes
            ...
            if (load == null || load.size() <= 0) { // In an unfortunate event that client logged in before load
                lastUpdate = 0;
                c.SendPacket(LoginPacket.CheckPasswordResult(c, LoginResult.ALREADY_LOGGEDIN));
                return;
            }
        ...
        if (c.finishLogin() == 0) {
            c.SendPacket(LoginPacket.CheckPasswordResult(c, LoginResult.SUCCESS));
