...
public abstract class MapleEvent {
    ...
    public static final void setEvent(final ServerOdinGame cserv, final boolean auto) {
        if (auto) {
            for (MapleEventType t : MapleEventType.values()) {
                ...
                if (e.isRunning) {
                    for (int i : e.mapid) {
                        if (cserv.getEvent() == i) {
                            e.broadcast(ResWrapper.BroadCastMsgNoticeOld("The event will start in 30 seconds!"));
    ...
    public static final void mapLoad(final MapleCharacter chr, final int channel) {
        ...
        for (MapleEventType t : MapleEventType.values()) {
            final MapleEvent e = ServerOdinGame.getInstance(channel).getEvent(t);
            if (e.isRunning) {
    ...
    public static final String scheduleEvent(final MapleEventType event, final ServerOdinGame cserv) {
        ...
        World.Broadcast.broadcastMessage(ResWrapper.BroadCastMsgNoticeOld("Hello AsteriaSEA! Let's play a " + String.valueOf(event) + " event in channel " + cserv.getChannel() + "! Change to channel " + cserv.getChannel() + " and use @event command!").getBytes());
        return "";
    }
}
