...
public class MapleInventoryManipulator {
    ...
    public static short addbyItem(final MapleClient c, final IItem item, final boolean fromcs) {
        ...
        if (newSlot == -1) {
            if (!fromcs) {
                c.getSession().write(WrapCWvsContext.updateInv());
                c.getSession().write(ResWrapper.getShowInventoryFull());
            }
            return newSlot;
        }
        if (!fromcs) {
            c.SendPacket(ResWrapper.addInventorySlot(type, item));
        }
    ...
    public static byte addId(MapleClient c, int itemId, short quantity, String owner, MaplePet pet, long period) {
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
                    ...
                    while (quantity > 0) {
                        if (i.hasNext()) {
                            ...
                            if (oldQ < slotMax && (eItem.getOwner().equals(owner) || owner == null) && eItem.getExpiration() == -1) {
                                ...
                                c.getSession().write(ResWrapper.updateInventorySlot(type, eItem, false));
                            }
                        } else {
                            break;
                        }
            ...
            if (quantity == 1) {
                ...
                c.SendPacket(ResWrapper.addInventorySlot(type, nEquip));
            } else {
    ...
    public static IItem addbyId_Gachapon(final MapleClient c, final int itemId, short quantity) {
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
                    ...
                    while (quantity > 0) {
                        if (i.hasNext()) {
                            ...
                            if (oldQ < slotMax) {
                                ...
                                c.getSession().write(ResWrapper.updateInventorySlot(type, nItem, false));
                            }
                        } else {
                            break;
                        }
            ...
            if (quantity == 1) {
                ...
                c.SendPacket(ResWrapper.addInventorySlot(type, item, true));
                c.getPlayer().havePartyQuest(item.getItemId());
                return item;
            } else {
                throw new InventoryException("Trying to create equip with non-one quantity");
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
            ...
            if (!GameConstants.isRechargable(item.getItemId())) {
                    ...
                    while (quantity > 0) {
                        if (i.hasNext()) {
                            ...
                            if (oldQ < slotMax && item.getOwner().equals(eItem.getOwner()) && item.getExpiration() == eItem.getExpiration()) {
                                ...
                                quantity -= (newQ - oldQ);
                                eItem.setQuantity(newQ);
                                c.getSession().write(ResWrapper.updateInventorySlot(type, eItem, true));
                            }
                        } else {
                            break;
                        }
                ...
                while (quantity > 0) {
                    ...
                    c.SendPacket(ResWrapper.addInventorySlot(type, nItem, true));
                }
            } else {
                // Throwing Stars and Bullets - Add all into one slot regardless of quantity.
    ...
    public static void equip(final MapleClient c, final short src, short dst) {
        ...
        if (stats.get("equipTradeBlock") == 1) { // Block trade when equipped.
            ...
            if (!ItemFlag.UNTRADEABLE.check(flag)) {
                ...
                source.setFlag(flag);
                c.getSession().write(ResWrapper.updateSpecialItemUse_(source, GameConstants.getInventoryType(source.getItemId()).getType()));
            }
        }
    ...
    public static void unequip(final MapleClient c, final short src, final short dst) {
        ...
        c.getSession().write(ResWrapper.moveInventoryItem(MapleInventoryType.EQUIP, src, dst, (byte) 1));
    ...
    public static boolean drop(final MapleClient c, MapleInventoryType type, final short src, short quantity, final boolean npcInduced) {
        ...
        if (quantity < source.getQuantity() && !GameConstants.isRechargable(source.getItemId())) {
            ...
            c.getSession().write(ResWrapper.dropInventoryItemUpdate(type, source));
            ...
            c.getSession().write(ResWrapper.dropInventoryItem((src < 0 ? MapleInventoryType.EQUIP : type), src));
            if (src < 0) {
                c.getPlayer().equipChanged();
            }
