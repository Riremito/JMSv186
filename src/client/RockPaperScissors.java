...
public class RockPaperScissors {
    ...
    public RockPaperScissors(final MapleClient c, final byte mode) {
        c.getSession().write(ResCRPSGameDlg.getRPSMode((byte) (0x09 + mode), -1, -1, -1));
        if (mode == 0) {
    ...
    public final boolean answer(final MapleClient c, final int answer) {
        if (ableAnswer && !win && answer >= 0 && answer <= 2) {
            ...
            if (response == answer) {
                c.getSession().write(ResCRPSGameDlg.getRPSMode((byte) 0x0B, -1, (byte) response, (byte) round));
                ...
                c.getSession().write(ResCRPSGameDlg.getRPSMode((byte) 0x0B, -1, (byte) response, (byte) (round + 1)));
                ableAnswer = false;
                win = true;
            } else { //they lose
                c.getSession().write(ResCRPSGameDlg.getRPSMode((byte) 0x0B, -1, (byte) response, (byte) -1));
                ableAnswer = false;
            }
            return true;
        }
    ...
    public final boolean timeOut(final MapleClient c) {
        if (ableAnswer && !win) {
            ableAnswer = false;
            c.getSession().write(ResCRPSGameDlg.getRPSMode((byte) 0x0A, -1, -1, -1));
            return true;
        }
        reward(c);
        return false;
    ...
    public final boolean nextRound(final MapleClient c) {
        if (win) {
            ...
            if (round < 10) {
                win = false;
                ableAnswer = true;
                c.getSession().write(ResCRPSGameDlg.getRPSMode((byte) 0x0C, -1, -1, -1));
                return true;
            }
        }
        reward(c);
