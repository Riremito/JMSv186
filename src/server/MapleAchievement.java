...
public class MapleAchievement {
    ...
    public void finishAchievement(MapleCharacter chr) {
        ...
        if (notice && !chr.isGM()) {
            World.Broadcast.broadcastMessage(ResWrapper.BroadCastMsgNotice("[Achievement] Congratulations to " + chr.getName() + " on " + name + " and rewarded with " + reward + " A-cash!").getBytes());
        } else {
            chr.getClient().getSession().write(ResWrapper.BroadCastMsgEvent("[Achievement] You've gained " + reward + " A-cash as you " + name + "."));
        }
    }
}
