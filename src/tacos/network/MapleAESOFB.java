...
public class MapleAESOFB {
    ...
    // CIGCipher::bShuffle
    private static final byte[] CIGCipher_bShuffle = new byte[]{
    ...
    private static final byte[] CAESCipher_bDefaultAESKeyValue = new byte[]{
    ...
    // CAESCipher::Decrypt
    private boolean CAESCipher_Decrypt(byte[] data, int offset, int nLen) {
        ...
        if (Content.OldIV.get()) {
            if (Region.TWMS.check() || Region.HKMS.check()) {
                ...
                myIv = CAESCipher_AES_DecInit_HKMS5();
            } else {
                // iv is never changed.
                myIv = CAESCipher_AES_DecInit_JMS131();
            }
        } else {
            // iv is never changed.
            myIv = CAESCipher_AES_DecInit();
        }
    ...
    // CAESCipher::AES_DecInit
    private byte[] CAESCipher_AES_DecInit() {
        ...
        if (this.iv != null) {
            ...
            for (int i = 0; i < ChainVar.length; i++) {
                ChainVar[i] = CAESCipher_bDefaultAESKeyValue[i];
            }
        }
    ...
    private byte[] CAESCipher_AES_DecInit_JMS131() {
        byte[] ChainVar = new byte[16];
        ...
        if (this.iv != null) {
            ...
            for (int i = 0; i < ChainVar.length; i++) {
                ChainVar[i] = CAESCipher_bDefaultAESKeyValue[3]; // F2
            }
        }
    ...
    private byte[] CAESCipher_AES_DecInit_HKMS5() {
        byte[] ChainVar = new byte[16];

        if (this.iv != null) {
            byte[] iv_copy = (this.iv_old_hkms == null) ? this.iv.clone() : this.iv_old_hkms.clone();
            for (int i = 0; i < 4; i++) {
                CIGCipher_MorphKey(CIGCipher_bShuffle[i], iv_copy);
            ...
            for (int i = 0; i < ChainVar.length; i++) {
                ChainVar[i] = CAESCipher_bDefaultAESKeyValue[i]; // C65053F2A8...
            }
        }
